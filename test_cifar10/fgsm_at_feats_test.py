...
def fgsm_at_feats_test(seed=0, device=0, alpha=0.6, name="test", T=8.0, label_smoothing=0.5):
    device = torch.device(
    ...
    fgsm_uap = FGSM_AT_Feats(model, device=device, seed=seed, num_class=num_class, name=name)
    fgsm_uap.train(
        opt,
        scheduler,
        train_loader,
        ...
        alpha=alpha,
        T=T,
        label_smoothing=label_smoothing,uap_num=50
    )
