...
from fat.fgsm_at import FGSM_AT
from models import MyResNet18
from utils.dataset import train_transform, test_transform


def fgsm_at_test(seed=0, device=0, alpha=0.6, name="test", T=8.0, label_smoothing=0.5):
    device = torch.device(
    ...
    fgsm_uap = FGSM_AT(model, device=device, seed=seed, num_class=num_class, name=name)
    fgsm_uap.train(
        opt,
        scheduler,
        train_loader,
