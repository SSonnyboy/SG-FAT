...
class FGSM_AT_Feats(ATBase):
    ...
    def __init__(
        ...
        device=None,
        seed=0,
        num_class=10,uap_num=50
    ):
        super(FGSM_AT_Feats, self).__init__(
    ...
    def train(
        ...
        momentum = torch.ones((uap_num, *image_shape)).to(self.device) * 0.9
        feature_layer = FeatureLayer(uap_num=uap_num, class_num=num_classes).to(self.device)
        ...
        for epoch in range(total_epoch):
            ...
            for images, labels in train_loader:
                ...
                self.model.eval()
                with torch.no_grad():
                    logits, feats = self.model(images, mid_fea=True)
                ...
                feature, out = feature_layer(feats[-1])
