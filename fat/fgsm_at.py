...
def soft_cross_entropy(logits, soft_targets):
    ...
    log_softmax = F.log_softmax(logits+1e-9, dim=1)
    loss = torch.sum(-soft_targets * log_softmax, dim=1).mean()
    return loss


class FGSM_AT(ATBase):
    ...
    def train(
        ...
        uaps = (
            ...
            .uniform_(-self.uap_eps, self.uap_eps)
            .to(self.device)
        )
        uaps = torch.clamp(self.uap_eps * torch.sign(uaps), -self.uap_eps, self.uap_eps)
        momentum = torch.ones(image_shape).to(self.device) * 0.9
        ...
        for epoch in range(total_epoch):
            ...
            for images, labels in train_loader:
                ...
                init = (
                    uaps.clone().unsqueeze(0).expand(B, -1, -1, -1)
                    - L_y_batch
                    + L_t_batch
                )
                ...
                uaps = torch.clamp(
                    uaps + self.uap_eps * torch.sign(momentum),
                    -self.uap_eps,
                    self.uap_eps,
                )
